    ...
    optimizer = torch.optim.Adam([
        {
           'params': model.parameters(),
            'lr':config['learning_rate_model']# TODO: optimizer params and learning rate for model (lr provided in config)
        },
        {
            'params':latent_vectors.parameters(),
            'lr':config['learning_rate_code'] # TODO: optimizer params and learning rate for latent code (lr provided in config)
        }
    ])
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if iteration % config['print_every_n'] == (config['print_every_n'] - 1):
                ...
                if train_loss < best_loss:
                    ...
                    torch.save(latent_vectors.state_dict(), f'EPNVAD/runs/{config["experiment_name"]}/latent_best.ckpt')
            ...
            if iteration % config['visualize_every_n'] == (config['visualize_every_n'] - 1):
                ...
                for latent_idx in range(latent_vectors_for_vis.shape[0]):
                    ...
                    evaluate_model_on_grid(model, latent_vectors_for_vis[latent_idx, :], device, 64, f'EPNVAD/runs/{config["experiment_name"]}/meshes/{iteration:05d}_{latent_idx:03d}.obj')
