...
class InferenceHandler3DEPN:
    ...
    def infer_single(self, input_sdf, target_df):
        ...
        input_sdf_temp = np.clip(input_sdf, a_min=-3, a_max=3)
        input_sdf_temp = np.reshape(np.stack([np.abs(input_sdf_temp), np.sign(input_sdf_temp)]),(1,2,32,32,32))
        with torch.no_grad():
            reconstructed_df = self.model(torch.from_numpy(input_sdf_temp))
