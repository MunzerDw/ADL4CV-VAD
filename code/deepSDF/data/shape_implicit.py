...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    dataset_path = Path("data/sdf_sofas")  # path to sdf data for ShapeNet sofa class - make sure you've downloaded the processed data at appropriate path
    ...
    def __init__(self, num_sample_points, split):
        ...
        self.items = Path(f"data/splits/sofas/{split}.txt").read_text().splitlines()  # keep track of shape identifiers based on split
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        pos_num = np.random.normal(loc=0.5, scale= 0.02, size=1)*self.num_sample_points
        pos_num_int = int(pos_num[0])
        neg_num_int = self.num_sample_points - pos_num_int
        idx_pos = np.random.choice(np.arange(len(pos_tensor)), pos_num_int, replace=False)
        idx_neg = np.random.choice(np.arange(len(neg_tensor)), neg_num_int, replace=False)
        return torch.cat([pos_tensor[idx_pos], neg_tensor[idx_neg]],dim=0)
