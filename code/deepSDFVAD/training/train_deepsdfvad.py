...
def train(model, latent_vectors, latent_log_var, train_dataloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            #print(latent_log_var.shape)
            #print(batch_x_vad.shape)
            ...
            #batch_latent_log_var = latent_log_var[batch['indices']].unsqueeze(1).expand(-1, batch['points'].shape[1], -1)
            #batch_latent_log_var = batch_latent_log_var.reshape((num_points_per_batch, config['latent_code_length']))

            predicted_sdf = model(torch.cat([batch_x_vad, points], dim=1))
            ...
            #if epoch > 100:
            #    total_loss = total_loss + code_regularization
            total_loss.backward()
            optimizer.step()
            # loss logging
            ...
            if iteration % config['print_every_n'] == (config['print_every_n'] - 1):
                ...
                print(f'[{epoch:03d}/{batch_idx:05d}] train_loss: '
                      f'{train_loss:.6f} kl_loss: {kl_loss:.6f} normal_loss: {loss:.6f}')
...
def main(config):
    ...
    # Instantiate model
    model = DeepSDFVAD(config['latent_code_length'])
    # Instantiate latent vectors for each training shape
    #latent_vectors = torch.tensor(np.random.normal(0, 0.001, size=(len(train_dataset),config['latent_code_length'])), dtype=torch.float,
    ...
    # Start training
    train(model, latent_vectors, latent_log_var, train_dataloader, device, config)
