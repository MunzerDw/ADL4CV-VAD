...
class InferenceHandlerDeepSDF:
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            batch_indices = random.sample(range(len(points)), self.num_samples)
    ...
    def interpolate(self, shape_0_id, shape_1_id, num_interpolation_steps):
        ...
        for i in range(0, num_interpolation_steps + 1):
            ...
            interpolated_code = (latent_codes[0,:]*i + latent_codes[1,:]*(num_interpolation_steps-i))/ num_interpolation_steps
    ...
    def infer_from_latent_code(self, latent_code_index):
        ...
        vertices, faces, sdf = evaluate_model_on_grid(model, latent_codes[0], self.device, 64, None)

        return vertices, faces, latent_codes[0]

    def infer_from_latent_code_random(self, latent_code_index):
        model = self.get_model()
        train_latent_codes = self.get_latent_codes()
        latent_code_indices = torch.LongTensor([latent_code_index]).to(self.device)
        latent_codes = train_latent_codes(latent_code_indices)
        random_latent_code = torch.randn_like(latent_codes[0]).to(self.device)
        vertices, faces, sdf = evaluate_model_on_grid(model, random_latent_code, self.device, 64, None)
        return vertices, faces, sdf

    def infer_from_latent_code_modify(self, latent_code_index, change_index, change_value):
        model = self.get_model()
        train_latent_codes = self.get_latent_codes()
        ...
        latent_code_indices = torch.LongTensor([latent_code_index]).to(self.device)
        latent_codes = train_latent_codes(latent_code_indices)
        latent_changed = latent_codes[0]
        for i in range(len(change_index)):
            latent_changed[change_index[i]] = change_value[i]
        # reconstruct the shape at latent code
        vertices, faces, sdf = evaluate_model_on_grid(model, latent_changed, self.device, 64, None)

        return vertices, faces, latent_changed
